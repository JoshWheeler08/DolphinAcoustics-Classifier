...
public class tfnode implements Comparable<tfnode>, Serializable
{
    ...
    transient LinkedList<FitPoly> active_fits;
    ...
    static public tfnode create(double time, double freq)
	//double dphase, double ddphase) {
{
	...
	if (free_list != null) {
		...
		node.init_common(time, freq,0,0); //, dphase, ddphase);
		// clear out the linked lists
		node.predecessors.clear();
		node.successors.clear();
	} else {
		node = new tfnode(time, freq, 0,0); //, dphase, ddphase);
	}
	return node;
}
    		...
    		double phase) //double dphase, double ddphase)
    {
    	...
    	/* Create a new node at the specified time, snr, freq, and phase */
    	this.time = time;
    	this.freq = freq;
    	this.snr = snr;
    	this.phase = phase;
    	...
    	active_fits = null;  // predictions associated with node
    }
    ...
    /**
     * Comparison based on frequency, then time
     * @param other
     * @return int
     */
    ...
    /**
     * Comparison based on time only
     * @param other
     * @return int
     */
    public int compareByTime(tfnode other) {
    ...
    public void set_fits(LinkedList<FitPoly> fits) {
    	/* Set of predictor fits based on this node's history */
    	active_fits = fits;
    }
    
    public LinkedList<FitPoly> get_fits() {
    	return active_fits;
    }
    
    public void clear_fits() {
    	/* Remove predictor fits when not needed */
    	active_fits = null;
    }
